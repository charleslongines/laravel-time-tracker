...
import { CheckCircle, Eye, EyeOff, XCircle } from 'lucide-vue-next';
...
const form = useForm({
    ...
    ip_address: props.user.ip_address || '',
    user_agent: props.user.user_agent || '',
});
...
// Auto-detect IP and User Agent (only if not already set)
const detectedIP = ref('');
const detectedUserAgent = ref('');

onMounted(async () => {
    // Only auto-detect if IP is not already set
    if (!form.ip_address) {
        try {
    ...
    // Only auto-detect if user agent is not already set
    if (!form.user_agent) {
...
const formErrors = computed(() => {
    ...
    if (form.password && passwordStrength.value.score < 2) {
        errors.password = 'Password is too weak';
    }

    if (form.password && !form.password_confirmation) {
        errors.password_confirmation = 'Please confirm your password';
    } else if (form.password && !passwordMatch.value) {
                ...
                <form @submit.prevent="submit" class="space-y-6 p-6">
                    ...
                    <div>
                        <label for="password" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                            Password <span class="text-gray-500 dark:text-gray-400">(leave blank to keep current)</span>
                        </label>
                        <div class="relative">
                            <input
                                id="password"
